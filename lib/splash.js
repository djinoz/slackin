...
export default function splash({ name, logo, active, total, channels, iframe }){
  let div = dom('<h1 style="width:80%;text-align:center;margin:auto;">Xamarin Evolve Slack</h1>', \ 
    '<h4>Share session notes and stay in touch with others.</h4>', \
    dom('.splash',
    !iframe && dom('.logos',
      logo && dom('.logo.org'),
      dom('.logo.slack')
    ...
    !iframe && dom('footer',
      '<h2>Sponsored by</h2> \
      <a href="http://streethawk.com/evolve/" target="_blank"><img style="width:60%" src="https://dashboard.streethawk.com/dashboard/public/img/global/2015-sh-logo.png"/></a><br/><br/> \
      powered by ',
